...
module.exports = class RemoteRequest {
   ...
   * @param {*} opts
   */
  static getDomainServerIPList(opts = {}, cache) {
    return _asyncToGenerator(function* () {
      let harmonyIP = Util.getHarmonyIP(cache);
      ...
      if (!harmonyIP) {
        ...
        if (res && res.code === 0 && res.data && res.data.length) {
          ...
          // 缓存
          Util.setHarmonyIP(data, cache);
          harmonyIP = Util.getHarmonyIP(cache);
        }
      }
   ...
   */
  static getRandomDomainIP(domain = '', cache) {
    return _asyncToGenerator(function* () {
      if (!domain) {
        return {};
      }
      const options = Object.assign({}, Util.getHarmonyConfig(cache), {
        domain
      });
      const res = yield RemoteRequest.getDomainServerIPList(options, cache);
   ...
   * @return {}
   */
  static getDataWithUrl(options, cache) {
    return _asyncToGenerator(function* () {
      if (!options.url) {
        return {};
      }
      ...
      // 取domain对应的Harmony地址
      const serverIP = yield RemoteRequest.getRandomDomainIP(domain, cache);
      // 组装成新的api发送请求
